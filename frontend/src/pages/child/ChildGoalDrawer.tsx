...
export default function ChildGoalDrawer({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const handleRequest = async (goalId: string) => {
    ...
    appToast.success(t('child.goal_request_success', '申请成功！请等待家长确认 🎁'));
    fetchGoals();
  };
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-t-[32px] max-h-[85vh] flex flex-col p-5 pb-10 shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-black dark:text-white">{ t('child.goal_my_title', '我的心愿单') }</h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-1">
          ...
          {!showForm ? (
            <button onClick={() => setShowForm(true)} className="w-full py-4 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl flex items-center justify-center gap-2 text-gray-500 font-bold">
              <Plus size={20} /> { t('child.goal_add_new', '许下一个新愿望') }
            </button>
          ) : (
            <div className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-3xl space-y-4">
              <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder={ t('child.goal_name_placeholder', '想要什么？') } className="w-full p-3 rounded-xl dark:bg-gray-800 dark:text-white outline-none font-bold" />
              <input type="number" value={form.target_points} onChange={e => setForm({...form, target_points: parseInt(e.target.value)})} placeholder={ t('child.goal_target_points_placeholder', '需要多少分？') } className="w-full p-3 rounded-xl dark:bg-gray-800 dark:text-white outline-none font-bold" />
              <button onClick={async () => {
                await service.post('/goals', form);
                setShowForm(false);
                fetchGoals();
              }} className="w-full py-3 bg-blue-600 text-white font-black rounded-xl shadow-lg shadow-blue-200">{ t('child.goal_add_new_btn', '添加愿望') }</button>
            </div>
          )}
                ...
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-gray-500">{goal.current_points} / {goal.target_points} {currentFamily?.point_emoji || '🪙'}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {goal.status === 'completed' && (
                      <button onClick={() => handleRequest(goal.id)} className="bg-green-500 text-white px-4 py-2 rounded-xl text-xs font-black shadow-lg shadow-green-200">{ t('child.goal_request_btn', '申请兑换') }</button>
                    )}
                    {goal.status === 'pending' && <span className="text-orange-500 text-xs font-black bg-orange-50 px-3 py-1.5 rounded-xl">{ t('child.goal_pending_status', '等待家长核准...') }</span>}
