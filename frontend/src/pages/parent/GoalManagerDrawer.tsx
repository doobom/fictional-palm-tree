...
export default function GoalManagerDrawer({ isOpen, onClose, child, onSuccess }: any) {
  ...
  const handleActivate = async (goalId: string) => {
    try {
      ...
      fetchGoals();
    } catch (e) { appToast.error( t('parent.goal_activate_failed') ); }
  };

  const handleRedeem = async (goalId: string) => {
    if (!window.confirm( t('parent.goal_redeem_confirm') )) return;
    try {
      await service.post('/goals/redeem', { childId: child.id, goalId });
      appToast.success( t('parent.goal_redeem_success') );
      fetchGoals();
      if (onSuccess) onSuccess(); 
    } catch (e) { appToast.error( t('parent.goal_redeem_failed') ); }
  };

  const handleCreate = async () => {
    if (!form.name.trim() || form.target_points <= 0) return appToast.error( t('parent.goal_create_failed_need_more_info') );
    try {
      await service.post('/goals', { childId: child.id, ...form });
      setShowForm(false);
      setForm({ name: '', target_points: 100, emoji: '🚲' });
      fetchGoals();
    } catch (e) { appToast.error(t('parent.goal_create_failed')); }
  };
      ...
      <div className={`absolute inset-x-0 bottom-0 bg-gray-50 dark:bg-gray-900 shadow-2xl pb-safe rounded-t-[24px] transition-all duration-300 transform ${isOpen ? 'translate-y-0' : 'translate-y-full'} flex flex-col max-h-[85vh]`}>
        ...
        <div className="px-5 flex items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-3 transition-colors">
          <div className="flex items-center gap-2 text-gray-900 dark:text-gray-100">
            ...
            <h3 className="text-lg font-bold">{ t('parent.goals_title', { name: child.name }) }</h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {!showForm ? (
            ...
            >
              <Plus size={20} /> { t('parent.goal_create') }
            </button>
          ) : (
            <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-blue-100 dark:border-blue-900/30 space-y-3">
              <div className="flex justify-between items-center">
                <span className="font-bold text-sm text-gray-600 dark:text-gray-300">{ t('parent.goal_details') }</span>
                <span onClick={() => setShowForm(false)} className="text-gray-400 text-xs underline cursor-pointer">{ t('common.cancel') }</span>
              </div>
              <div className="flex gap-2">
                ...
                <input 
                  ...
                  placeholder={ t('parent.goal_name_placeholder') } 
                />
              </div>
              <input 
                type="number" 
                ...
                placeholder={ t('parent.goal_target_points_placeholder') } 
              />
              <button 
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isCompleted && (
                      ...
                      >
                        <Gift size={14} /> { t('parent.goal_redeem_btn') }
                      </button>
                    )}
                    {isRedeemed && <span className="text-xs font-black text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-lg">{ t('parent.goal_redeemed') }</span>}
                  </div>
                </div>
