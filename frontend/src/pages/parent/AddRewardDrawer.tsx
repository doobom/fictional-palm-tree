...
export default function AddRewardDrawer({ isOpen, onClose, onSuccess, existingData }: AddRewardDrawerProps) {
  ...
  // 🌟 核心修复 3：完全移除 if (!isOpen) return null;
    ...
    <div className={`fixed inset-0 z-[9999] ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      <div className={`absolute inset-x-0 bottom-0 bg-white rounded-t-[24px] flex flex-col transform transition-transform duration-300 ease-out max-h-[90vh] ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        <div className="px-5 pb-2 relative">
          ...
          <h3 className="text-xl font-semibold text-gray-900">{existingData ? '编辑奖品' : '新增奖品'}</h3>
        </div>
          ...
          <form id="rewardForm" onSubmit={handleSubmit} className="space-y-4">
            <div className="flex space-x-3">
              <div className="w-1/4">
                ...
                <input type="text" maxLength={2} value={emoji} onChange={(e) => setEmoji(e.target.value)} className="h-12 w-full px-2 text-center text-2xl rounded-xl bg-gray-100 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-colors" />
              </div>
              <div className="w-3/4">
                <label className="block text-sm font-medium text-gray-700 mb-1">名称</label>
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：看电视一小时" className="h-12 w-full px-4 rounded-xl bg-gray-100 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-colors font-medium" />
              </div>
            </div>
            ...
            <div className="flex space-x-3">
              <div className="w-1/2">
                ...
                <input type="number" required min="1" value={cost} onChange={(e) => setCost(e.target.value)} placeholder="0" className="h-12 w-full px-4 rounded-xl bg-gray-100 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-colors font-medium" />
              </div>
              <div className="w-1/2">
                <label className="block text-sm font-medium text-gray-700 mb-1">库存限制</label>
                <input type="number" min="1" value={stock} onChange={(e) => setStock(e.target.value)} placeholder="∞ (不限)" className="h-12 w-full px-4 rounded-xl bg-gray-100 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-colors font-medium" />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">所属分类</label>
              <select value={categoryId} onChange={e => setCategoryId(e.target.value)} className="h-12 w-full px-4 rounded-xl bg-gray-100 focus:ring-2 focus:ring-blue-500 outline-none border-none">
                <option value="">未分类</option>
                {categories.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
              </select>
            </div>

            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl mt-2">
              <div>
                <p className="font-medium text-gray-900 text-sm">需要家长审批</p>
                <p className="text-xs text-gray-500 mt-0.5">兑换后需家长同意才生效</p>
              </div>
        ...
        <div className="p-5 pt-2" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 20px) + 20px)' }}>
          <button type="submit" form="rewardForm" disabled={loading} className="w-full py-3.5 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-200 disabled:text-gray-400 text-white rounded-xl font-semibold text-lg transition-colors">
            {loading ? '提交中...' : '保存奖品'}
          </button>
        </div>
      </div>
